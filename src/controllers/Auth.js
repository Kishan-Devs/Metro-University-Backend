...
exports.signUp = async (req, res) => {
    ...
    try {
        ...
        // console.log("Collection:", Otp.collection.name);

        const allOtps = await Otp.find({});
        // console.log("All OTPs:", allOtps);

        const existOtp = await Otp.findOne({ email }).sort({ createdAt: -1 });

        // console.log("Matched OTP:", existOtp);

        // console.log("OTP:", existOtp);
        ...
        if (String(otp) !== String(existOtp.otp)) {
            return res.status(400).json({
                success: false,
                message: "Invalid otp",
            });
